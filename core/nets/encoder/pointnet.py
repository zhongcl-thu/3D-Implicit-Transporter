...
from .pointnetpp_attn import PointNetPlusPlusAttnFusion_cross
import ipdb
...
class LocalPoolPointnetPPFusion(nn.Module):
    ...
    def __init__(
        ...
        self.feat_pos = PointNetPlusPlusAttnFusion_cross(
    ...
    def pool_and_3dconv(self, coord, index, net):
        net = self.blocks[0](net)
        for block in self.blocks[1:]:
            pooled = self.pool_local(coord, index, net)
            net = torch.cat([net, pooled], dim=2)
    ...
    def forward(self, p, p2):
        ...
        # acquire the index for each point
        coord, index = self.get_coord_index(p)
        coord2, index2 = self.get_coord_index(p2)
        
        net, net2 = self.feat_pos(p, p2, return_score=self.return_score)
        ...
        fea2 = {}
        fea2['grid'] = self.generate_grid_features(p2, c2, unet3d=self.unet3d)
