...
def viz_pc_keypoint(pc, keypoints, kp_radius=0.3, save_path=None, color_v=[0.792, 0.792, 0.792]):
    ...
    for i in range(keypoints.shape[0]):
        ...
        mesh_sphere.paint_uniform_color(palette[i]) #kp['semantic_id']
...
def viz_ply_keypoint(textured_mesh, keypoints, radius, color_v=[0.792, 0.792, 0.792], save_path=None):
    # draw ply and keypoints
    textured_mesh = o3d.io.read_triangle_mesh('exp/bullet_multi_joint/kps6_std0.15_pad0_input5k_atten_simple_seed111_pose_corr_0.01_pose_axis_0.001/test_result/seen/87.ply')
    textured_mesh.compute_vertex_normals()
    # vertices = np.array(textured_mesh.vertices)
    # faces = np.array(textured_mesh.triangles)
    ...
    for i in range(keypoints.shape[0]):
        ...
        #mesh_sphere.translate(face_coords.T @ kp['mesh_info']['face_uv'])
        mesh_sphere.translate(kp)
        mesh_sphere.paint_uniform_color(palette[i])
        mesh_spheres.append(mesh_sphere)
        
    print('visualizing ply mesh with keypoints highlighted')
    #o3d.visualization.draw_geometries([textured_mesh, *mesh_spheres])
    custom_draw_geometry_with_key_callback([textured_mesh, *mesh_spheres], save_path)
...
def draw_matching_lines(pc1, pc2, kp1, kp2, corr):
    points = np.concatenate((kp1, kp2), 0)
    lines = corr
    lines[:, 1] += kp1.shape[0]
    # points = [
    #     [0, 0, 0],
    ...
    colors1[:, 1] = 1
    pcd1 = make_o3d_pcd(pc1, colors=colors1)
    ...
    if pc2 is None:
        ...
        colors2[:, 2] = 1
        pcd2 = make_o3d_pcd(pc2, colors=colors2)
