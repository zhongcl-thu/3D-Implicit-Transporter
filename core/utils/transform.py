...
def axis2transform(axis, up=np.array([0.0, 0.0, 1.0])):
    v = np.cross(up, axis)
    ...
    return Transform(Rotation.from_matrix(R), np.zeros(3))


def get_transform(pos, quat):
    return Transform(Rotation.from_quat(np.array(quat)), np.array(pos))
...
def transform_from_screw(l, m, theta, t):
    p = np.cross(l, m)
    rotation = rotation_matrix_from_axis(l, theta)
    translation = -rotation.dot(p) + p + t * l
