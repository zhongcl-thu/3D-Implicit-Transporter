...
class Solver:
    ...
    def create_model(self):
        ...
        if model_path != None:
            try:
                ...
                checkpoint = torch.load(model_path, self.device) #, lambda a,b:a
                self.model.load_state_dict({k[7:]:v for k,v in checkpoint.items()}, strict=False)
    ...
    def logging(self, mode='train'):
        ...
        if mode == 'train':
            self.logger.info(
                ...
                "occup_loss_transp:{occup_loss_transp:.3f}  "
                "occup_loss_ori:{occup_loss_ori:.3f}  "
                "pose_loss:{pose_loss:.3f}  "
                ...
                "recall:{recall:.3f}  ".format(
                    ...
                    occup_loss_transp=float(tmp.log['occup_loss_transp'].avg) \
                        if tmp.log.get('occup_loss_transp', None) is not None else 0,
                    occup_loss_ori=float(tmp.log['occup_loss_ori'].avg) \
                        if tmp.log.get('occup_loss_ori', None) is not None else 0,
                    pose_loss=float(tmp.log['pose_loss'].avg) \
                        if tmp.log.get('pose_loss', None) is not None else 0,
            ...
            self.logger.info(
                ...
                "recall:{recall:.3f}  ".format(
                    ...
                    occup_loss_transp=float(self.mean(tmp.stats_val['occup_loss_transp'])) \
                                if tmp.stats_val.get('occup_loss_transp', None) is not None else 0,
                    occup_loss_ori=float(self.mean(tmp.stats_val['occup_loss_ori'])) \
                                if tmp.stats_val.get('occup_loss_ori', None) is not None else 0,
                    pose_loss=float(self.mean(tmp.stats_val['pose_loss'])) \
                        if tmp.stats_val.get('pose_loss', None) is not None else 0,
    ...
    def analyse_result(self, details, outputs2):
        ...
        if 'occ_transp' in outputs2.keys():
            prediction = torch.where(outputs2['occ_transp'][:, :N] > 0.8, 1.0, 0.0)
    ...
    def forward(self):
        ...
        self.analyse_result(details, outputs)

        return l, details, outputs
